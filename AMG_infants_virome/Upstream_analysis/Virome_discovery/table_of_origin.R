...
if (length(temp) == 0) {
	  cat("No files matching the specified pattern found. Exiting script.\n")
  q("no")
}

# Filter out empty files
temp <- temp[sapply(temp, function(x) file.info(x)$size > 0)]

# Check if there are any non-empty files
if (length(temp) == 0) {
	  cat("All files are empty. Exiting script.\n")
  q("no")
}
