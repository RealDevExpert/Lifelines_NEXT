...
genera_filt <- genera[(rowSums(genera!=0) > 0.05*ncol(genera)), ]
...
vOTUS_assigned_BacGen_filt <- vOTUS_assigned_BacGen[(rowSums(vOTUS_assigned_BacGen!=0) > 0.05*ncol(vOTUS_assigned_BacGen)),]
...
# ANALYSIS
##############################
genera_phenos <- mixed_models_taxa(MGS_metadata, "Short_sample_ID_bact", genera_filt_CLR, c("infant_sex","infant_gestational_age", "infant_birthweight",
                                                                                             "infant_place_delivery","infant_mode_delivery","mother_age_years",
                                                                                             "infant_ever_never_breastfed","infant_feeding_mode_imputed_B_to_M3","infant_food_solid_intro_M6",
                                                                                             "Age_months", "perc_reads_aligned","viral_richness_MGS",
                                                                                             "viral_alpha_diversity_MGS","temperate_richness_MGS","temperate_RA_MGS",
                                                                                             "bacterial_alpha_diversity", "infant_ffq_feeding_mode_complex", "infant_ffq_feeding_mode_simple"))

dynamic_genera <- genera_phenos[genera_phenos$FDR < 0.05 & genera_phenos$Pheno=='Age_months',]

# selection of the most dynamic genera
sort(abs(dynamic_genera$Estimate), decreasing=T)[10] #cut off on Estimate to choose top-10

# need to add two other genera that are also dynamic (FDR < 0.05, but do not express the highest slope)
additional_dynamic <- c('k__Bacteria|p__Proteobacteria|c__Gammaproteobacteria|o__Enterobacterales|f__Enterobacteriaceae|g__Klebsiella',
                        'k__Bacteria|p__Actinobacteria|c__Actinobacteria|o__Bifidobacteriales|f__Bifidobacteriaceae|g__Bifidobacterium')

dynamic_plot <- genera_filt_CLR[,c(dynamic_genera[abs(dynamic_genera$Estimate) > 0.3319692,]$Bug, additional_dynamic)]
...
pdf('./04.PLOTS/Dynamic_bacterial_genera_infants.pdf', width=16/2.54, height=14/2.54)
...
genera_phenos_FDR <- genera_phenos[genera_phenos$FDR < 0.05,]

# selected genera dependant on feeding mode:
...
birth_plot <- genera_filt_CLR[,unique(genera_phenos_FDR[genera_phenos_FDR$Pheno %in% c('infant_place_delivery', 'infant_birthweight'),]$Bug)]
colnames(birth_plot) <- gsub('.*g__', '', colnames(birth_plot))
birth_plot<- merge(birth_plot, MGS_metadata[,c("Timepoint", 'infant_place_delivery')], by='row.names')

pdf('./04.PLOTS/Genus_Akkermansia_infants_birthplace.pdf', width=12/2.54, height=9/2.54)
ggplot(birth_plot, aes(Timepoint, Akkermansia, fill=infant_place_delivery)) + 
  labs (y="CLR transformed abundance", x="Timepoint") + 
  geom_boxplot(outlier.shape = NA, alpha=0.5) + 
  geom_sina(colour='#303841', size=0.6) +
  theme_bw()+
...
vOTUs_gen_phenos <- mixed_models_taxa(VLP_metadata, "Short_sample_ID", vOTUS_assigned_BacGen_filt_CLR, c("infant_sex","infant_gestational_age", "infant_birthweight",
                                                                                            "infant_place_delivery","infant_mode_delivery","mother_age_years",
                                                                                            "infant_ever_never_breastfed","infant_feeding_mode_imputed_B_to_M3","infant_food_solid_intro_M6",
                                                                                            "Age_months", "perc_reads_aligned","viral_richness",
                                                                                            "viral_alpha_diversity","temperate_richness","temperate_RA",
                                                                                            "bacterial_alpha_diversity", "infant_ffq_feeding_mode_complex", "infant_ffq_feeding_mode_simple"))
dynamic_vOTUs_gen <- vOTUs_gen_phenos[vOTUs_gen_phenos$FDR < 0.05 & vOTUs_gen_phenos$Pheno=='Age_months',]
...
# since both found g__Clostridium are from different families:
dynamic_vOTUs_plot <- dynamic_vOTUs_plot[,-grep('g__Clostridium', colnames(dynamic_vOTUs_plot))]
...
colors_phages <- c("#FF0000", "#ff0080", "#FFA500", "#0000FF", "#008000", "#800000", "#008080", "#DC143C", "#6A5ACD")
...
###### OUTPUT #####
write.table(dynamic_genera, '03a.RESULTS/DYNAMIC_TOP_PREVALENT_RA_GENERA_FIRST_YEAR_NEXT_PILOT_12_05_2023.txt', sep='\t', row.names = F, quote=F)
write.table(genera_phenos_FDR, '03a.RESULTS/TOP_PREVALENT_RA_GENERA_phenos_mixed_models.txt', sep='\t', row.names = F, quote=F)
write.table(vOTUs_gen_phenos_FDR, '03a.RESULTS/TOP_PREVALENT_vOTUs_aggr_Bac_Gen_phenos_mixed_models.txt', sep='\t', row.names = F, quote=F)
